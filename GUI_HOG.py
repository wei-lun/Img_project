...
def load_image(image_path):
    ...
    img = cv2.resize(img, (128, 128))  # 調整大小到 128x128
    return img
...
def predict_image(image_path, unet_model, svm_model, knn_model):
    ...
    segmented_image = unet_model.predict(image)[0]
    segmented_image = (segmented_image * 255).astype('uint8')

    if len(segmented_image.shape) == 3 and segmented_image.shape[2] == 1:
        segmented_image = np.squeeze(segmented_image)

    feature_vector = extract_hog_features([segmented_image])[0]
...
if __name__ == "__main__":
    ...
    svm_model = load('svm_classifier_HOG.pkl')
    knn_model = load('knn_classifier_HOG.pkl')
